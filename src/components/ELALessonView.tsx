...
import {
  PartsOfSpeechGameSim,
  SentenceTypeSorterSim,
  FigurativeLangIdentifierSim,
  StoryElementsExplorerSim,
  MainIdeaBuilderSim,
  POVIdentifierSim,
  ParagraphBuilderSim,
  EssayMapSim,
} from '../simulations/ela';

const simMap: Record<string, React.ComponentType> = {
  'parts-of-speech-game': PartsOfSpeechGameSim,
  'sentence-type-sorter': SentenceTypeSorterSim,
  'figurative-lang-identifier': FigurativeLangIdentifierSim,
  'story-elements-explorer': StoryElementsExplorerSim,
  'main-idea-builder': MainIdeaBuilderSim,
  'pov-identifier': POVIdentifierSim,
  'paragraph-builder': ParagraphBuilderSim,
  'essay-map': EssayMapSim,
};
...
export default function ELALessonView({ topic, alreadyCompleted, onComplete, onBack }: Props) {
      ...
      <div className="flex border-b border-slate-800 px-6">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-5 py-3 text-sm font-medium border-b-2 transition ${tab === t.id ? 'border-rose-500 text-rose-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        {tab === 'lesson' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
              <div className="text-xs font-bold uppercase tracking-widest text-rose-400 mb-3">Quick Summary</div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setTab('simulation')}
                className="flex-1 py-3 rounded-xl bg-rose-500/20 border border-rose-500/40 text-rose-400 font-semibold hover:bg-rose-500/30 transition">
                Try the Simulation →
              </button>
        ...
        {tab === 'simulation' && (
          <div className="space-y-4">
            <p className="text-slate-400 text-sm">Work through the interactive exercise below to practise the concept.</p>
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
              {Sim ? <Sim /> : <p className="text-slate-500 text-center py-10">Simulation coming soon</p>}
            </div>
            <button onClick={() => setTab('quiz')}
              className="w-full py-3 rounded-xl bg-rose-500 hover:bg-rose-400 font-semibold transition">
              Ready? Take the Quiz →
            </button>
          </div>
        )}
        ...
        {tab === 'quiz' && (
          <div>
            ...
            <Quiz topic={topic} onComplete={handleQuizComplete} subjectId="ela" />
            {quizDone && (
              <button onClick={onBack}
