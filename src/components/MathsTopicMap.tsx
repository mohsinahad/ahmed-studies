import { mathsCurriculum, isMathsTopicUnlocked } from '../data/mathsCurriculum';
...
export default function MathsTopicMap({ completedTopics, scores, onSelect, onReset, onBackToHome }: Props) {
  const units = [...new Set(mathsCurriculum.map((t) => t.unit))];
  const total = mathsCurriculum.length;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      ...
      <div className="max-w-3xl mx-auto mb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <h1 className="text-3xl font-bold text-white">Mathematics</h1>
            <p className="text-slate-400 text-sm mt-1">Grades 6–8 · Numbers, Geometry & Algebra</p>
          </div>
          <button
            onClick={onReset}
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        {units.map((unit) => {
          const topics = mathsCurriculum.filter((t) => t.unit === unit);
          ...
          return (
            <div key={unit}>
              ...
              <div className="grid gap-3">
                {topics.map((topic) => {
                  const idx = mathsCurriculum.findIndex((t) => t.id === topic.id);
                  const unlocked = isMathsTopicUnlocked(idx, completedTopics);
