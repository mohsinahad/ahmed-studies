...
const simMap: Record<string, React.ComponentType> = {
  ...
  states: StatesSim,
  changes: ChangesSim,
  'mass-conservation': MassConservationSim,
...
export default function ChemLessonView({ topic, alreadyCompleted, onComplete, onBack }: Props) {
      ...
      <div className="flex border-b border-slate-800 px-6">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-5 py-3 text-sm font-medium border-b-2 transition ${tab === t.id ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        {tab === 'lesson' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
              <div className="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-3">Quick Summary</div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setTab('simulation')}
                className="flex-1 py-3 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 font-semibold hover:bg-emerald-500/30 transition">
                Try the Simulation →
              </button>
        ...
        {tab === 'simulation' && (
          <div className="space-y-4">
            <p className="text-slate-400 text-sm">Interact with the simulation below. Change the values and observe how chemistry works in real time.</p>
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
              {Sim ? <Sim /> : <p className="text-slate-500 text-center py-10">Simulation coming soon</p>}
            </div>
            <button onClick={() => setTab('quiz')}
              className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 font-semibold transition">
              Ready? Take the Quiz →
            </button>
          </div>
        )}
