import { chemistryCurriculum, isChemTopicUnlocked } from '../data/chemistryCurriculum';
...
export default function ChemTopicMap({ completedTopics, scores, onSelect, onReset, onBackToHome }: Props) {
  const units = [...new Set(chemistryCurriculum.map((t) => t.unit))];
  const total = chemistryCurriculum.length;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      ...
      <div className="max-w-3xl mx-auto mb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <h1 className="text-3xl font-bold text-white">Chemistry Explorer</h1>
            <p className="text-slate-400 text-sm mt-1">Grades 6–8 · Complete topics in order to unlock the next</p>
          </div>
          <button
            onClick={onReset}
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        {units.map((unit) => {
          const topics = chemistryCurriculum.filter((t) => t.unit === unit);
          ...
          return (
            <div key={unit}>
              ...
              <div className="grid gap-3">
                {topics.map((topic) => {
                  const idx = chemistryCurriculum.findIndex((t) => t.id === topic.id);
                  const unlocked = isChemTopicUnlocked(idx, completedTopics);
